...
function agregarItemAlFinalDeSlArray(array, elemento) {
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  for ( i = 0 ; i < arguments.length ; i++) {
    argumentsMult = argumentsMult * arguments[i];
  } 
  return argumentsMult;
}
...
for (i = 0 ; i < arreglo.length; i++) {
  if(arreglo[i] > 18) nuevaArray.push (numero[i]);
}
return nuevaArray.length;
}
...
function empiezaConNueve(n) {
  ...
  for (i = 0; i < n.lenght; i++) {
    if(nuevoString[0] === '9') return true;
  }
  return false;
}
...
function continueStatement(numero) {
  ...
  for (i = 0 ; i < 10 ; i++) {
    if ( i === 5) continue;
    else conteo = conteo + 2;
    newArray.push (conteo); 
  }
  return newArray;
}
